...
static char *ngx_rtmp_gop_cache_merge_app_conf(ngx_conf_t *cf, void *parent,
                                               void *child);
...
static ngx_command_t ngx_rtmp_gop_cache_commands[] = {
    {ngx_string("gop_cache"), NGX_RTMP_APP_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_flag_slot, NGX_RTMP_APP_CONF_OFFSET,
     offsetof(ngx_rtmp_gop_cache_app_conf_t, gop_cache), NULL},

    {ngx_string("gop_max_frame_count"), NGX_RTMP_APP_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, NGX_RTMP_APP_CONF_OFFSET,
     offsetof(ngx_rtmp_gop_cache_app_conf_t, gop_max_frame_count), NULL},

    {ngx_string("gop_max_video_count"), NGX_RTMP_APP_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, NGX_RTMP_APP_CONF_OFFSET,
     offsetof(ngx_rtmp_gop_cache_app_conf_t, gop_max_video_count), NULL},

    {ngx_string("gop_max_audio_count"), NGX_RTMP_APP_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, NGX_RTMP_APP_CONF_OFFSET,
     offsetof(ngx_rtmp_gop_cache_app_conf_t, gop_max_audio_count), NULL},
...
ngx_module_t ngx_rtmp_gop_cache_module = {NGX_MODULE_V1,
...
static ngx_int_t ngx_rtmp_gop_cache_alloc_cache(ngx_rtmp_session_t *s) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                 "gop alloc cache: gop_cache_count='%uD'",
                 ctx->gop_cache_count);
...
static void ngx_rtmp_gop_cache_frame(ngx_rtmp_session_t *s, ngx_uint_t prio,
  ...
  if (ch->type == NGX_RTMP_MSG_VIDEO) {
    ...
    if (prio != NGX_RTMP_VIDEO_KEY_FRAME && ctx->cache_head == NULL) {
      ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                     "drop video non-keyframe timestamp='%uD'", ch->timestamp);
  ...
  if (ctx->video_frame_in_all == 0 && ch->type == NGX_RTMP_MSG_AUDIO) {
    ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "drop audio frame timestamp='%uD'", ch->timestamp);
...
static void ngx_rtmp_gop_cache_send(ngx_rtmp_session_t *s) {
  ...
  if (ctx == NULL || ctx->stream == NULL || ctx->stream->pub_ctx == NULL ||
      !ctx->stream->publishing) {
    return;
  }
...
static ngx_int_t ngx_rtmp_gop_cache_publish(ngx_rtmp_session_t *s,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                 "gop cache publish: name='%s' type='%s'", v->name, v->type);
  ...
  if (ctx->pool == NULL) {
    ctx->pool =
        ngx_create_pool(NGX_GOP_CACHE_POOL_CREATE_SIZE, s->connection->log);
...
static ngx_int_t ngx_rtmp_gop_cache_play(ngx_rtmp_session_t *s,
  ...
  ngx_log_debug4(
      ...
      "gop cache play: name='%s' start='%i' duration='%i' reset='%d'", v->name,
      (ngx_int_t)v->start, (ngx_int_t)v->duration, (ngx_uint_t)v->reset);
