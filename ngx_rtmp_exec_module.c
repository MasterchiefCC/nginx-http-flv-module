...
static ngx_int_t ngx_rtmp_exec_merge_confs(ngx_array_t *conf,
                                           ngx_array_t *prev) {
  size_t n;
...
static void ngx_rtmp_exec_child_dead(ngx_event_t *ev) {
  ...
  ngx_log_error(
      NGX_LOG_INFO, e->log, 0, "exec: child %ui exited; %s", (ngx_int_t)e->pid,
      e->respawn_timeout == NGX_CONF_UNSET_MSEC ? "respawning" : "ignoring");
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_RTMP, e->log, 0, "exec: schedule respawn %Mmsec",
                 e->respawn_timeout);
...
static ngx_int_t ngx_rtmp_exec_kill(ngx_rtmp_exec_t *e, ngx_int_t kill_signal) {
  ...
  if (kill(e->pid, kill_signal) == -1) {
    ngx_log_error(NGX_LOG_INFO, e->log, ngx_errno, "exec: kill failed pid=%i",
                  (ngx_int_t)e->pid);
  } else {
...
static ngx_int_t ngx_rtmp_exec_run(ngx_rtmp_exec_t *e) {
  ...
  if (e->managed) {
    if (e->active) {
      ngx_log_debug1(NGX_LOG_DEBUG_RTMP, e->log, 0, "exec: already active '%V'",
                     &ec->cmd);
      return NGX_OK;
    }
    ...
    if (ret == -1) {
      ...
      ngx_log_error(NGX_LOG_INFO, e->log, ngx_errno, "exec: fcntl failed");
...
static ngx_int_t ngx_rtmp_exec_init_ctx(ngx_rtmp_session_t *s,
  ...
  if (push_conf->nelts > 0) {
    if (ngx_array_init(&ctx->push_exec, s->connection->pool, push_conf->nelts,
                       sizeof(ngx_rtmp_exec_t)) != NGX_OK) {
      return NGX_ERROR;
    }
...
static ngx_int_t ngx_rtmp_exec_init_pull_ctx(ngx_rtmp_session_t *s,
  ...
  for (; *ppctx; ppctx = &(*ppctx)->next) {
    ...
    if (pctx->name.len == len && ngx_strncmp(name, pctx->name.data, len) == 0) {
      goto done;
    }
  }
...
static ngx_int_t ngx_rtmp_exec_close_stream(ngx_rtmp_session_t *s,
  ...
  if (ctx->push_exec.nelts > 0) {
    ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "exec: delete %uz push command(s)", ctx->push_exec.nelts);
  ...
  if (pctx && --pctx->counter == 0) {
    ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "exec: delete %uz pull command(s)", pctx->pull_exec.nelts);
