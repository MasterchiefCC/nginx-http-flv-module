...
ngx_int_t ngx_rtmp_user_message_handler(ngx_rtmp_session_t *s,
  ...
  if (b->last - b->pos < 6) {
    ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "too small buffer for user message: %d", b->last - b->pos);
    return NGX_OK;
  }
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                 "RTMP recv user evt %s (%i)", ngx_rtmp_user_message_type(evt),
                 (ngx_int_t)evt);
...
ngx_int_t ngx_rtmp_aggregate_message_handler(ngx_rtmp_session_t *s,
  ...
  while (in) {
    ...
    ngx_log_debug6(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   ...
                   ngx_rtmp_message_type(ch.type), (ngx_int_t)ch.type, ch.mlen,
                   ch.timestamp, timestamp - base_time, ch.msid);
...
ngx_int_t ngx_rtmp_amf_message_handler(ngx_rtmp_session_t *s,
  ...
  if ((h->type == NGX_RTMP_MSG_AMF3_SHARED ||
       h->type == NGX_RTMP_MSG_AMF3_META || h->type == NGX_RTMP_MSG_AMF3_CMD) &&
  ...
  /* skip name */
  in = act.link;
  if (ngx_strcasecmp(func, (u_char *)"onMetaData")) {
    h->mlen -= act.offset;
