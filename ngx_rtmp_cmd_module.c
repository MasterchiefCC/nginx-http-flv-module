...
static void ngx_rtmp_parse_url_anti_key(ngx_rtmp_session_t *s, u_char *args,
                                        u_char *key, u_char *time);
static ngx_int_t ngx_rtmp_check_anti_time(ngx_rtmp_session_t *s,
                                          u_char *anti_attack_time);
static ngx_int_t ngx_rtmp_anti_attack(ngx_rtmp_session_t *s, u_char *args,
...
static ngx_int_t ngx_rtmp_cmd_connect_init(ngx_rtmp_session_t *s,
  ...
  static ngx_rtmp_amf_elt_t in_cmd[] = {
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("swfUrl"), v.swf_url, sizeof(v.swf_url)},
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("serverName"), v.server_name,
...
static ngx_int_t ngx_rtmp_cmd_connect(ngx_rtmp_session_t *s,
  ...
  static ngx_rtmp_amf_elt_t out_inf[] = {
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("code"), "NetConnection.Connect.Success",
       0},

      {NGX_RTMP_AMF_STRING, ngx_string("description"), "Connection succeeded.",
       0},

      {NGX_RTMP_AMF_NUMBER, ngx_string("objectEncoding"), &object_encoding, 0}};
...
static void ngx_rtmp_parse_url_anti_key(ngx_rtmp_session_t *s, u_char *args,
  ...
  //////////////key//////////////
  start = ngx_strstr(args, "key=");
  if (start == NULL) {
    return;
  }

  start += ngx_strlen("key=");
  end = ngx_strchr(start, '&');
  len = end == NULL ? args + ngx_strlen(args) - (u_char *)start : end - start;
  if (len >= NGX_RTMP_MD5_LEN) {
    return;
  }

  ngx_memcpy(key, start, len);

  //////////////time//////////////

  start = ngx_strstr(args, "time=");
  if (start == NULL) {
    return;
  }

  start += ngx_strlen("time=");
  end = ngx_strchr(start, '&');
  len = end == NULL ? args + ngx_strlen(args) - (u_char *)start : end - start;
  if (len >= NGX_RTMP_MAX_HEX_TIME_LEN) {
    return;
  }

  ngx_memcpy(time, start, len);
}
...
static ngx_int_t ngx_rtmp_check_anti_time(ngx_rtmp_session_t *s,
  ...
  if (lacf == NULL || lacf->anti_time_valid == 0) {
    return NGX_OK;
  }

  result = strtol((const char *)anti_attack_time, &end, 16);
  ngx_uint_t gap = abs(result * 1000 - ngx_current_msec);

  if (end && *end == '\0' && gap <= lacf->anti_time_valid) {
    return NGX_OK;
  }

  ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "invalid time key");
  return NGX_ERROR;
}
...
static ngx_int_t ngx_rtmp_anti_attack(ngx_rtmp_session_t *s, u_char *args,
  ...
  char data[512] = {'\0'};
  char *cur = NULL;
  unsigned char md[NGX_RTMP_MD5_LEN];
  char buf[NGX_RTMP_MD5_STR_LEN] = {'\0'};
  char tmp[3] = {'\0'};
  ngx_uint_t anti_check = 0, anti_key_len = 0, anti_time_len = 0,
             stream_name_len = 0, i;
  u_char anti_attack_key[NGX_RTMP_MD5_STR_LEN],
      anti_attack_time[NGX_RTMP_MAX_HEX_TIME_LEN];
  ngx_rtmp_live_app_conf_t *lacf;
  ...
  if (s->flashver.data &&
      ngx_strnstr(s->flashver.data, "LNX.11", s->flashver.len)) {
    return NGX_OK;
  }

  ngx_rtmp_parse_url_anti_key(s, args, anti_attack_key, anti_attack_time);

  anti_check = (publisher && lacf->anti_publish_attack &&
                lacf->anti_publish_identify.len) ||
                       (!publisher && lacf->anti_play_attack &&
                        lacf->anti_play_identify.len)
                   ? 1
                   : 0;
  anti_key_len = ngx_strlen(anti_attack_key);
  anti_time_len = ngx_strlen(anti_attack_time);
  stream_name_len = ngx_strlen(s->stream_name);

  if (anti_check == 0) {
    return NGX_OK;
  }

  if (anti_key_len > 0 &&  // has attack key in url
      ((lacf->anti_time_use == 0 &&
        anti_time_len == 0) ||       // not using anti_time_use
       (lacf->anti_time_use != 0 &&  // using anti_time_use
        anti_time_len > 0 &&
        ngx_rtmp_check_anti_time(s, anti_attack_time) == NGX_OK))) {
    if (lacf->anti_key_whitelist.len == anti_key_len &&
        ngx_strncasecmp(anti_attack_key, lacf->anti_key_whitelist.data,
                        lacf->anti_key_whitelist.len) == 0) {
      return NGX_OK;
    }
    u_char *target_key;
    size_t target_len;
    target_key = (publisher == 1) ? lacf->anti_publish_identify.data
                                  : lacf->anti_play_identify.data;
    target_len = (publisher == 1) ? lacf->anti_publish_identify.len
                                  : lacf->anti_play_identify.len;

    ngx_memcpy(data, target_key, target_len);
    ngx_memcpy(cur, s->stream_name, stream_name_len);
    cur = data + target_len + stream_name_len;

    if (lacf->anti_time_use != 0) {
      ngx_memcpy(cur, anti_attack_time, ngx_strlen(anti_attack_time));
    } else if (stream_name_len > 0) {
      if (stream_name_len >= 2) *cur++ = s->stream_name[stream_name_len - 2];
      *cur = s->stream_name[stream_name_len - 1];
    }

    ngx_md5_init(&ctx);
    ngx_md5_update(&ctx, data, ngx_strlen(data));
    ngx_md5_final(md, &ctx);
    for (i = 0; i < 16; i++) {
      sprintf(tmp, "%02x", md[i]);
      strcat(buf, tmp);
    }
    if (ngx_strcasecmp(anti_attack_key, (u_char *)buf) != 0) {
...
static ngx_int_t ngx_rtmp_cmd_publish_init(ngx_rtmp_session_t *s,
  ...
  ngx_memcpy(s->stream_name, v.name, ngx_strlen(v.name));

  // ngx_rtmp_cmd_is_record(s, v.args);

  ngx_rtmp_anti_attack(s, v.args, 1);
...
static ngx_int_t ngx_rtmp_cmd_play_init(ngx_rtmp_session_t *s,
  ...
  static ngx_rtmp_amf_elt_t in_elts[] = {
      ...
      {NGX_RTMP_AMF_OPTIONAL | NGX_RTMP_AMF_BOOLEAN, ngx_null_string, &v.reset,
       0}};
  ...
  ngx_rtmp_anti_attack(s, v.args, 0);
...
static ngx_int_t ngx_rtmp_cmd_play2_init(ngx_rtmp_session_t *s,
  ...
  static ngx_rtmp_amf_elt_t in_obj[] = {
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("streamName"), &v.name, sizeof(v.name)},
  };
