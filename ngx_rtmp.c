...
static ngx_int_t ngx_rtmp_find_virtual_server(
    ngx_connection_t *c, ngx_rtmp_virtual_names_t *virtual_names,
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
      ...
      if (module->merge_app_conf) {
        ...
        rv = ngx_rtmp_merge_applications(cf, &cscf->applications,
                                         cscfp[s]->ctx->app_conf, module, mi);
        if (rv != NGX_CONF_OK) {
          *cf = pcf;
          return rv;
        }
...
static ngx_int_t ngx_rtmp_init_events(ngx_conf_t *cf,
  ...
  if (ngx_array_init(&cmcf->amf, cf->pool, 1, sizeof(ngx_rtmp_amf_handler_t)) !=
      NGX_OK) {
    return NGX_ERROR;
  }
...
static ngx_int_t ngx_rtmp_init_event_handlers(ngx_conf_t *cf,
  ...
  static size_t amf_events[] = {
      NGX_RTMP_MSG_AMF_CMD,  NGX_RTMP_MSG_AMF_META,  NGX_RTMP_MSG_AMF_SHARED,
      NGX_RTMP_MSG_AMF3_CMD, NGX_RTMP_MSG_AMF3_META, NGX_RTMP_MSG_AMF3_SHARED};
...
static ngx_listening_t *ngx_rtmp_add_listening(ngx_conf_t *cf,
  ...
  ls =
      ngx_create_listening(cf, &addr->opt.sockaddr.sockaddr, addr->opt.socklen);
  if (ls == NULL) {
    return NULL;
  }
...
ngx_int_t ngx_rtmp_set_virtual_server(ngx_rtmp_session_t *s, ngx_str_t *host) {
  ...
  rc = ngx_rtmp_find_virtual_server(
      s->connection, rconn->addr_conf->virtual_names, host, s, &cscf);
  ...
  if (dcscf->out_queue != cscf->out_queue) {
    ...
    s->out = ngx_pcalloc(
        s->out_pool,
        sizeof(ngx_chain_t *) * ((ngx_rtmp_core_srv_conf_t *)cscf->ctx
  ...
  if (dcscf->max_streams != cscf->max_streams) {
    ...
    ngx_memmove(in_streams, s->in_streams,
                sizeof(ngx_rtmp_stream_t) *
...
ngx_int_t ngx_rtmp_is_match_len(const char *src, ngx_uint_t src_len,
  ...
  ngx_uint_t i = 0, j = 0;
  ngx_uint_t ret = 0;
  if (src == NULL || dest == NULL) return ret;
  while (i != src_len && j != dest_len) {
    if (src[i] == '%') {
      i++;
      if (src[i] == '%') continue;

      while (j != dest_len && dest[j] != src[i]) {
        j++;
      }
