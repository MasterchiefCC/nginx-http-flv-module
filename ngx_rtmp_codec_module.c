...
#define NGX_RTMP_META_CHAT_TYPE 1
#define NGX_RTMP_META_PPT_TYPE 2
#define NGX_RTMP_META_INCLASS_TYPE 3
...
static ngx_int_t ngx_rtmp_codec_save_stream_meta(ngx_rtmp_session_t *s,
                                                 ngx_rtmp_header_t *head,
                                                 ngx_chain_t *in);
static ngx_int_t ngx_rtmp_codec_send_meta(ngx_rtmp_session_t *s,
                                          uint32_t timestamp);
...
static ngx_int_t ngx_rtmp_codec_disconnect(ngx_rtmp_session_t *s,
  ...
  if (ctx->meta_streaminfo) {
    ngx_rtmp_free_shared_chain(cscf, ctx->meta_streaminfo);
    ctx->meta_streaminfo = NULL;
  }
...
ngx_rtmp_codec_video_is_combined_nals(ngx_chain_t *in, ngx_rtmp_session_t *s) {
  ...
  if (in->buf->pos + 5 + ctx->avc_nal_bytes <= in->buf->last) {
    ...
    if (ctx->video_codec_id == NGX_RTMP_VIDEO_H264) {
      ...
      return (nal_type >= NGX_RTMP_NALU_SPS && nal_type <= NGX_RTMP_NALU_PPS)
                 ? NGX_OK
                 : NGX_ERROR;
    }
  }
...
static ngx_inline ngx_int_t ngx_rtmp_get_codec_header_type(
    ngx_rtmp_session_t *s, ngx_rtmp_header_t *h, ngx_chain_t *in) {
...
ngx_int_t ngx_rtmp_codec_parse_avc_header_in_keyframe(ngx_rtmp_session_t *s,
                                                      ngx_chain_t *in,
                                                      ngx_buf_t *out) {
  ngx_rtmp_codec_ctx_t *ctx;
  u_char *p;
  ...
  ctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_codec_module);
  if (ctx == NULL || ctx->video_codec_id != NGX_RTMP_VIDEO_H264 ||
      ctx->avc_nal_bytes == 0 || out == NULL) {
    return NGX_ERROR;
  }
  ...
  *b->last++ = 0xe1;
  ...
  while (in) {
    ...
    if (!(nal_type >= NGX_RTMP_NALU_SPS && nal_type <= NGX_RTMP_NALU_PPS)) {
      if (ngx_rtmp_hls_copy(s, NULL, &p, len - 1, &in) != NGX_OK) {
        return NGX_ERROR;
      }

      ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                     "codec: skip non-sps or non-pps, nal_type='%uD'",
                     nal_type);
...
void ngx_rtmp_codec_parse_avc_header_compat(ngx_rtmp_session_t *s,
                                            ngx_chain_t **in,
                                            ngx_chain_t *sps) {
...
static void ngx_rtmp_codec_parse_avc_header(ngx_rtmp_session_t *s,
  ...
  ngx_uint_t profile_idc, width, height, crop_left, crop_right, crop_top,
      crop_bottom, frame_mbs_only, n, cf_idc, num_ref_frames;
  ngx_rtmp_codec_ctx_t *ctx;
  ngx_rtmp_bit_reader_t br;
...
static void ngx_rtmp_codec_dump_header(ngx_rtmp_session_t *s, const char *type,
  ...
  for (pp = buf, p = in->buf->pos;
       p < in->buf->last && pp < buf + sizeof(buf) - 2; ++p) {
    *pp++ = hex[*p >> 4];
    *pp++ = hex[*p & 0x0f];
  }
...
static ngx_int_t ngx_rtmp_codec_reconstruct_meta(ngx_rtmp_session_t *s) {
  ...
  static ngx_rtmp_amf_elt_t out_inf[] = {
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("Server"),
       "NGINX RTMP (https://github.com/MasterchiefCC)", 0},
      ...
      {NGX_RTMP_AMF_NUMBER, ngx_string("videodatarate"), &v.video_data_rate, 0},

      {NGX_RTMP_AMF_NUMBER, ngx_string("videocodecid"), &v.video_codec_id, 0},

      {NGX_RTMP_AMF_NUMBER, ngx_string("audiodatarate"), &v.audio_data_rate, 0},
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("xmetadata"), &v.xmetadata,
       sizeof(v.xmetadata)},

      {NGX_RTMP_AMF_STRING, ngx_string("xesstr"), &v.xesstr, sizeof(v.xesstr)},
  };
  ...
  ngx_memcpy(v.xmetadata, ctx->xmetadata, sizeof(ctx->xmetadata));
  ngx_memcpy(v.xesstr, ctx->xesstr, sizeof(ctx->xesstr));
  ...
  return ngx_rtmp_codec_send_meta(s, 0);
}
...
static ngx_int_t ngx_rtmp_codec_save_stream_meta(ngx_rtmp_session_t *s,
  ...
  ctx->meta_streaminfo = ngx_rtmp_append_shared_bufs(cscf, NULL, in);

  ngx_memzero(&h, sizeof(h));
  h.csid = NGX_RTMP_CSID_AMF;
  h.msid = NGX_RTMP_MSID;
  h.type = NGX_RTMP_MSG_AMF_META;
  h.timestamp = head->timestamp;
  ngx_rtmp_prepare_message(s, &h, NULL, ctx->meta_streaminfo);
...
static ngx_int_t ngx_rtmp_codec_send_meta(ngx_rtmp_session_t *s,
  ...
  ngx_rtmp_codec_prepare_meta(s, timestamp);

  for (pctx = ctx->stream->ctx; pctx; pctx = pctx->next) {
    if (pctx == ctx || pctx->paused ||
        codec_ctx->meta_version == pctx->meta_version) {
      continue;
    }
    ss = pctx->session;
    ngx_log_debug0(NGX_LOG_DEBUG_RTMP, ss->connection->log, 0, "live: meta");

    if (ngx_rtmp_send_message(ss, codec_ctx->meta, 0) == NGX_OK) {
      pctx->meta_version = codec_ctx->meta_version;
    }
  }
...
static ngx_int_t ngx_rtmp_codec_meta_data(ngx_rtmp_session_t *s,
  ...
  static struct {
    ...
    u_char questionId[64];
    u_char date[64];
    double timeout;
    u_char operateId[64];
    u_char num[64];

    u_char questionType[64];
    u_char url[512];
    double category;
  } v;
  ...
  static ngx_rtmp_amf_elt_t in_inf[] = {
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("metaType"), &v.metaType,
       sizeof(v.metaType)},

      {NGX_RTMP_AMF_NUMBER, ngx_string("type"), &v.type, 0},

      {NGX_RTMP_AMF_STRING, ngx_string("path"), &v.path, sizeof(v.path)},

      {NGX_RTMP_AMF_STRING, ngx_string("filename"), &v.filename,
       sizeof(v.filename)},

      {NGX_RTMP_AMF_STRING, ngx_string("questionId"), &v.questionId,
       sizeof(v.questionId)},

      {NGX_RTMP_AMF_STRING, ngx_string("date"), &v.date, sizeof(v.date)},

      {NGX_RTMP_AMF_NUMBER, ngx_string("time"), &v.timeout, 0},

      {NGX_RTMP_AMF_STRING, ngx_string("operateId"), &v.operateId,
       sizeof(v.operateId)},

      {NGX_RTMP_AMF_STRING, ngx_string("num"), &v.num, sizeof(v.num)},

      {NGX_RTMP_AMF_STRING, ngx_string("questionType"), &v.questionType,
       sizeof(v.questionType)},

      {NGX_RTMP_AMF_STRING, ngx_string("url"), &v.url, sizeof(v.url)},

      {NGX_RTMP_AMF_NUMBER, ngx_string("category"), &v.category, 0},
  };
  ...
  ngx_rtmp_record_metadata_start(s, h, in);
  ...
  ngx_memcpy(ctx->xmetadata, v.xmetadata, sizeof(v.xmetadata));
  ngx_memcpy(ctx->xesstr, v.xesstr, sizeof(v.xesstr));
  ...
  ngx_log_debug8(
      ...
      ngx_rtmp_get_video_codec_name(ctx->video_codec_id), ctx->video_codec_id,
      ngx_rtmp_get_audio_codec_name(ctx->audio_codec_id), ctx->audio_codec_id);

  ctx->action_type = 0;
  ngx_memcpy(ctx->metaType, v.metaType, sizeof(v.metaType));
  ngx_memcpy(ctx->actionid, v.questionId, sizeof(v.questionId));
  ngx_memcpy(ctx->questionType, v.questionType, sizeof(v.questionType));
  ngx_memcpy(ctx->url, v.url, sizeof(v.url));
  ngx_memcpy(ctx->date, v.date, sizeof(v.date));
  ctx->timer = ((ngx_uint_t)v.timeout) * 60;

  ngx_uint_t isAction = 0;
  if (ngx_strcasecmp(ctx->metaType, (u_char *)"changeStudyType") == 0) {
    ngx_uint_t pre_type = ctx->type, cur_type;
    ctx->type = cur_type = (ngx_uint_t)v.type;

    if (cur_type == NGX_RTMP_META_INCLASS_TYPE) {
      ngx_memcpy(ctx->record_path, v.path, sizeof(v.path));
      ngx_memcpy(ctx->record_filename, v.filename, sizeof(v.filename));
      ngx_rtmp_record_xueersi_start(s);
    } else if (pre_type == NGX_RTMP_META_INCLASS_TYPE &&
               cur_type == NGX_RTMP_META_CHAT_TYPE) {
      ngx_rtmp_record_xueersi_stop(s);
    }
  }

  if (ngx_strcasecmp(ctx->metaType, (u_char *)"questionType") == 0) {
    ctx->action_type = 1;
    isAction = 1;
  } else if (ngx_strcasecmp(ctx->metaType, (u_char *)"grantGoldType") == 0) {
    ctx->action_type = 2;
    isAction = 1;
    ngx_memcpy(ctx->actionid, v.operateId, sizeof(v.operateId));
  } else if (ngx_strcasecmp(ctx->metaType,
                            (u_char *)"answer_statistics_type") == 0) {
    ctx->action_type = 3;
    isAction = 1;
  } else if (ngx_strcasecmp(ctx->metaType, (u_char *)"examType") == 0) {
    switch ((ngx_uint_t)v.type) {
      case 1:
        ctx->action_type = 4;
        break;
      case 3:
    ...
    isAction = 1;
    ngx_memcpy(ctx->actionid, v.num, sizeof(v.num));
  } else if (ngx_strcasecmp(ctx->metaType, (u_char *)"audioTest") == 0) {
    switch ((ngx_uint_t)v.type) {
      case 1:
        ctx->action_type = 8;
        break;
    ...
    isAction = 1;
    ngx_memcpy(ctx->actionid, v.num, sizeof(v.num));
  } else if (ngx_strcasecmp(ctx->metaType, (u_char *)"H5Courseware") == 0) {
    switch ((ngx_uint_t)v.type) {
      case 1:
        ctx->action_type = 10;
        break;
  ...
  if ((ngx_uint_t)v.category >= MIN_NEW_CATEGORY) {
    ctx->action_type = (ngx_uint_t)v.category;
    isAction = 1;
  }

  if (isAction) {
    ngx_rtmp_record_metadata(s, h);
  }
  ...
  switch (cacf->meta) {
    ...
    case NGX_RTMP_CODEC_META_COPY:
      if (v.width && v.height) {
        ngx_rtmp_codec_save_stream_meta(s, h, in);
      }
