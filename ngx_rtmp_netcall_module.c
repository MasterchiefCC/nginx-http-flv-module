...
static void ngx_rtmp_netcall_recv(ngx_event_t *rev) {
  ...
  for (;;) {
    if (cs->inlast == NULL || cs->inlast->buf->last == cs->inlast->buf->end) {
      if (cs->in && cs->sink) {
        if (!cs->detached) {
...
ngx_chain_t *ngx_rtmp_netcall_http_format_request(
  ...
  b = ngx_create_temp_buf(
      pool, sizeof(rq_tmpl) + host->len + content_type->len + NGX_SIZE_T_LEN);
  if (b == NULL) {
    return NULL;
  }
  ...
  b->last = ngx_snprintf(b->last, b->end - b->last, rq_tmpl, host, content_type,
                         content_length);
...
ngx_chain_t *ngx_rtmp_netcall_http_format_session(ngx_rtmp_session_t *s,
  ...
  b = ngx_create_temp_buf(
      pool, sizeof("app=") - 1 + s->app.len * 3 + sizeof("&flashver=") - 1 +
                s->flashver.len * 3 + sizeof("&swfurl=") - 1 +
                s->swf_url.len * 3 + sizeof("&tcurl=") - 1 + s->tc_url.len * 3 +
  ...
  b->last = (u_char *)ngx_escape_uri(b->last, s->flashver.data, s->flashver.len,
                                     NGX_ESCAPE_ARGS);

  b->last = ngx_cpymem(b->last, (u_char *)"&swfurl=", sizeof("&swfurl=") - 1);
  b->last = (u_char *)ngx_escape_uri(b->last, s->swf_url.data, s->swf_url.len,
                                     NGX_ESCAPE_ARGS);

  b->last = ngx_cpymem(b->last, (u_char *)"&tcurl=", sizeof("&tcurl=") - 1);
  b->last = (u_char *)ngx_escape_uri(b->last, s->tc_url.data, s->tc_url.len,
                                     NGX_ESCAPE_ARGS);

  b->last = ngx_cpymem(b->last, (u_char *)"&pageurl=", sizeof("&pageurl=") - 1);
  b->last = (u_char *)ngx_escape_uri(b->last, s->page_url.data, s->page_url.len,
                                     NGX_ESCAPE_ARGS);
