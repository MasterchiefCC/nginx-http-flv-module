...
static ngx_int_t ngx_rtmp_script_add_capture_code(ngx_rtmp_script_compile_t *sc,
                                                  ngx_uint_t n);
#endif
static ngx_int_t ngx_rtmp_script_add_full_name_code(
...
ngx_int_t ngx_rtmp_compile_complex_value(
...
void ngx_rtmp_script_copy_code(ngx_rtmp_script_engine_t *e) {
  ...
  if (!e->skip) {
    e->pos =
        ngx_copy(p, e->ip + sizeof(ngx_rtmp_script_copy_code_t), code->len);
  }
...
static ngx_int_t ngx_rtmp_script_add_var_code(ngx_rtmp_script_compile_t *sc,
  ...
  code = ngx_rtmp_script_add_code(
      *sc->values, sizeof(ngx_rtmp_script_var_code_t), &sc->main);
  if (code == NULL) {
    return NGX_ERROR;
  }
...
static ngx_int_t ngx_rtmp_script_add_capture_code(ngx_rtmp_script_compile_t *sc,
  ...
  code = ngx_rtmp_script_add_code(
      *sc->lengths, sizeof(ngx_rtmp_script_copy_capture_code_t), NULL);
  if (code == NULL) {
    return NGX_ERROR;
  }

  code->code = (ngx_rtmp_script_code_pt)ngx_rtmp_script_copy_capture_len_code;
  code->n = 2 * n;

  code = ngx_rtmp_script_add_code(
      *sc->values, sizeof(ngx_rtmp_script_copy_capture_code_t), &sc->main);
  if (code == NULL) {
    return NGX_ERROR;
  }
...
static ngx_int_t ngx_rtmp_script_add_full_name_code(
  ...
  code = ngx_rtmp_script_add_code(
      *sc->lengths, sizeof(ngx_rtmp_script_full_name_code_t), NULL);
  if (code == NULL) {
    return NGX_ERROR;
  }
  ...
  code = ngx_rtmp_script_add_code(
      *sc->values, sizeof(ngx_rtmp_script_full_name_code_t), &sc->main);
  if (code == NULL) {
    return NGX_ERROR;
  }
...
static void ngx_rtmp_script_full_name_code(ngx_rtmp_script_engine_t *e) {
  ...
  if (ngx_get_full_name(e->request->connection->pool, prefix, &value) != NGX_OK)
#else
