...
static ngx_int_t ngx_rtmp_variable_request(ngx_rtmp_session_t *s,
                                           ...
                                           uintptr_t data);
static ngx_int_t ngx_rtmp_variable_request_get_size(
    ngx_rtmp_session_t *s, ngx_rtmp_variable_value_t *v, uintptr_t data);
...
static ngx_int_t ngx_rtmp_variable_host(ngx_rtmp_session_t *s,
                                        ngx_rtmp_variable_value_t *v,
                                        uintptr_t data);
static ngx_int_t ngx_rtmp_variable_binary_remote_addr(
    ngx_rtmp_session_t *s, ngx_rtmp_variable_value_t *v, uintptr_t data);
...
static ngx_rtmp_variable_t ngx_rtmp_core_variables[] = {
    ...
    {ngx_string("uri"), NULL, ngx_rtmp_variable_request,
     offsetof(ngx_rtmp_session_t, uri), NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("query_string"), NULL, ngx_rtmp_variable_request,
     offsetof(ngx_rtmp_session_t, args), NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("args"), ngx_rtmp_variable_set_args, ngx_rtmp_variable_request,
     offsetof(ngx_rtmp_session_t, args),
     NGX_RTMP_VAR_CHANGEABLE | NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("is_args"), NULL, ngx_rtmp_variable_is_args, 0,
     NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("server_name"), NULL, ngx_rtmp_variable_server_name, 0, 0, 0},

    {ngx_string("bytes_sent"), NULL, ngx_rtmp_variable_bytes_sent, 0, 0, 0},

    {ngx_string("request_time"), NULL, ngx_rtmp_variable_request_time, 0,
     NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("request_id"), NULL, ngx_rtmp_variable_request_id, 0, 0, 0},
    ...
    {ngx_string("connection"), NULL, ngx_rtmp_variable_connection, 0, 0, 0},

    {ngx_string("nginx_version"), NULL, ngx_rtmp_variable_nginx_version, 0, 0,
     0},

    {ngx_string("hostname"), NULL, ngx_rtmp_variable_hostname, 0, 0, 0},

    {ngx_string("pid"), NULL, ngx_rtmp_variable_pid, 0, 0, 0},

    {ngx_string("msec"), NULL, ngx_rtmp_variable_msec, 0,
     NGX_RTMP_VAR_NOCACHEABLE, 0},

#if (NGX_HAVE_TCP_INFO)
    {ngx_string("tcpinfo_rtt"), NULL, ngx_rtmp_variable_tcpinfo, 0,
     NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("tcpinfo_rttvar"), NULL, ngx_rtmp_variable_tcpinfo, 1,
     NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("tcpinfo_snd_cwnd"), NULL, ngx_rtmp_variable_tcpinfo, 2,
     NGX_RTMP_VAR_NOCACHEABLE, 0},

    {ngx_string("tcpinfo_rcv_space"), NULL, ngx_rtmp_variable_tcpinfo, 3,
     NGX_RTMP_VAR_NOCACHEABLE, 0},
#endif

    {ngx_string("arg_"), NULL, ngx_rtmp_variable_argument, 0,
     NGX_RTMP_VAR_NOCACHEABLE | NGX_RTMP_VAR_PREFIX, 0},

    {ngx_null_string, NULL, NULL, 0, 0, 0}};

ngx_rtmp_variable_value_t ngx_rtmp_variable_null_value = ngx_rtmp_variable("");
ngx_rtmp_variable_value_t ngx_rtmp_variable_true_value = ngx_rtmp_variable("1");
...
static ngx_int_t ngx_rtmp_variable_not_found(ngx_rtmp_session_t *s,
                                             ngx_rtmp_variable_value_t *v,
                                             uintptr_t data) {
  v->not_found = 1;
  return NGX_OK;
  ...
  for (i = 0; i < cmcf->variables.nelts; i++) {
    for (n = 0; n < cmcf->variables_keys->keys.nelts; n++) {
      ...
      if (v[i].name.len == key[n].key.len &&
        ...
        if (av->get_handler == NULL || (av->flags & NGX_RTMP_VAR_WEAK)) {
          break;
        }
