...
ngx_int_t ngx_rtmp_process_request_uri(ngx_rtmp_session_t *s) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0, "rtmp uri: \"%V\"",
                 &s->uri);

  ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0, "rtmp args: \"%V\"",
                 &s->args);
...
ngx_int_t ngx_rtmp_process_request_line(ngx_rtmp_session_t *s,
  ...
  if (s->stream.len) {
    ...
    if (args && args[0]) {
      *ngx_snprintf(s->request_line->pos, rlen + 1, "%V?%s", &s->tc_url, args) =
          CR;
    } else {
      *ngx_snprintf(s->request_line->pos, rlen + 1, "%V", &s->tc_url) = CR;
    }
  }
