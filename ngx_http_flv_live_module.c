...
ngx_rtmp_live_proc_handler_t *ngx_rtmp_live_proc_handlers[] = {
    &ngx_rtmp_live_proc_handler, &ngx_http_flv_live_proc_handler};
...
static ngx_command_t ngx_http_flv_live_commands[] = {
    {ngx_string("flv_live"), NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_flv_live_conf_t, flv_live), NULL},

    {ngx_string("poll_interval"), NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_flv_live_conf_t, poll_interval), NULL},

    ngx_null_command};

ngx_module_t ngx_http_flv_live_module = {NGX_MODULE_V1,
...
ngx_int_t ngx_http_flv_live_init(ngx_conf_t *cf) {
...
void *ngx_http_flv_live_create_loc_conf(ngx_conf_t *cf) {
...
char *ngx_http_flv_live_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                       void *child) {
...
ngx_int_t ngx_http_flv_live_init_handlers(ngx_cycle_t *cycle) {
...
ngx_int_t ngx_http_flv_live_init_process(ngx_cycle_t *cycle) {
...
ngx_int_t ngx_http_flv_live_send_header(ngx_rtmp_session_t *s) {
  ...
  for (i = 0; /* void */; i++) {
    ...
    if (ngx_strcasecmp(header[i].key.data, (u_char *)"connection") == 0) {
      connection_header = 1;
      if (ngx_strcasecmp(header[i].value.data, (u_char *)"keep-alive") == 0) {
        r->keepalive = 1;
      }
 ...
 **/
ngx_int_t ngx_http_flv_live_headers_filter(ngx_rtmp_session_t *s) {
  ngx_str_t value;
...
ngx_int_t ngx_http_flv_live_header_filter(ngx_rtmp_session_t *s) {
  u_char *p;
  size_t len;
  ...
  *b->last++ = LF;
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%*s",
...
void ngx_http_flv_live_send_tail(ngx_rtmp_session_t *s) {
  ngx_rtmp_core_srv_conf_t *cscf;
...
ngx_int_t ngx_http_flv_live_send_message(ngx_rtmp_session_t *s,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                 "flv live: HTTP send nmsg='%ui', priority='%ui' '#%ui'", nmsg,
                 priority, s->out_last);
...
ngx_int_t ngx_http_flv_live_request(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h,
                                    ngx_chain_t *in) {
...
ngx_int_t ngx_http_flv_live_play(ngx_rtmp_session_t *s, ngx_rtmp_play_t *v) {
  ngx_rtmp_live_app_conf_t *lacf;
  ...
  if (ngx_rtmp_process_request_line(
          s, v->name, v->args, (const u_char *)"flv live play") != NGX_OK) {
    r->main->count--;
  ...
  ngx_log_debug4(
      ...
      "flv live play: name='%s' start='%uD' duration='%uD' reset='%d'", v->name,
      (uint32_t)v->start, (uint32_t)v->duration, (uint32_t)v->reset);
...
void ngx_http_flv_live_play_handler(ngx_event_t *ev) {
  static ngx_rtmp_play_t v;
...
void ngx_http_flv_live_exec_pull_handler(ngx_event_t *ev) {
  ngx_connection_t *c;
  ngx_http_request_t *r;
  ngx_rtmp_session_t *s;
  ngx_http_flv_live_ctx_t *ctx;
...
void ngx_http_flv_live_read_handler(ngx_event_t *rev) {
  ngx_connection_t *c;
  ngx_http_request_t *r;
  ngx_rtmp_session_t *s;
  ngx_int_t n;
...
void ngx_http_flv_live_write_handler(ngx_event_t *wev) {
  ngx_connection_t *c;
  ngx_http_request_t *r;
  ngx_rtmp_session_t *s;
  ngx_int_t n;
...
ngx_int_t ngx_http_flv_live_preprocess(ngx_http_request_t *r,
                                       ngx_rtmp_connection_t *rconn) {
  ngx_http_flv_live_ctx_t *ctx;
  ngx_listening_t *ls;
...
ngx_rtmp_session_t *ngx_http_flv_live_init_connection(
    ngx_http_request_t *r, ngx_rtmp_connection_t *rconn) {
  ngx_rtmp_session_t *s;
  ngx_connection_t *c;
  void *data;
...
ngx_rtmp_session_t *ngx_http_flv_live_init_session(
    ngx_http_request_t *r, ngx_rtmp_addr_conf_t *addr_conf) {
  ngx_rtmp_session_t *s;
  ngx_rtmp_core_srv_conf_t *cscf;
  ...
  s->out = ngx_pcalloc(
      s->out_pool, sizeof(ngx_chain_t *) *
                       ((ngx_rtmp_core_srv_conf_t *)addr_conf->default_server
  ...
  s->in_streams = ngx_pcalloc(s->in_streams_pool,
...
ngx_int_t ngx_http_flv_live_connect_init(ngx_rtmp_session_t *s, ngx_str_t *app,
                                         ngx_str_t *stream) {
...
ngx_chain_t *ngx_http_flv_live_meta_message(ngx_rtmp_session_t *s,
                                            ngx_chain_t *in) {
  ngx_rtmp_core_srv_conf_t *cscf;
  ngx_http_request_t *r;
...
ngx_chain_t *ngx_http_flv_live_append_message(ngx_rtmp_session_t *s,
                                              ngx_rtmp_header_t *h,
                                              ngx_rtmp_header_t *lh,
                                              ngx_chain_t *in) {
  ngx_rtmp_core_srv_conf_t *cscf;
...
ngx_chain_t *ngx_http_flv_live_append_shared_bufs(
    ...
    ngx_flag_t chunked) {
  ngx_chain_t *tag, *chunk_head, *chunk_tail, chunk, *iter, *last_in, **tail,
      prev_tag_size;
  u_char *pos, *p,
#if !(NGX_WIN32)
...
void ngx_http_flv_live_free_message(ngx_rtmp_session_t *s, ngx_chain_t *in) {
  ngx_rtmp_core_srv_conf_t *cscf;
...
static void ngx_http_flv_live_close_session_handler(ngx_rtmp_session_t *s) {
  ngx_connection_t *c;
  ngx_rtmp_core_srv_conf_t *cscf;
...
ngx_int_t ngx_http_flv_live_handler(ngx_http_request_t *r) {
  ngx_int_t rc;
  ngx_http_flv_live_conf_t *hfcf;
  ngx_http_cleanup_t *cln;
  ngx_http_flv_live_ctx_t *ctx;
