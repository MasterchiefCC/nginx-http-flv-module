...
static void ngx_rtmp_amf_debug(const char *op, ngx_log_t *log, u_char *p,
  ...
  *hp = *sp = '\0';
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_RTMP, log, 0, "AMF %s (%d)%s '%s'", op, n, hstr,
                 str);
}
#endif
...
static ngx_int_t ngx_rtmp_amf_read_array(ngx_rtmp_amf_ctx_t *ctx,
  ...
  /* read length */
  if (ngx_rtmp_amf_get(ctx, buf, 4) != NGX_OK) return NGX_ERROR;
...
ngx_int_t ngx_rtmp_amf_write(ngx_rtmp_amf_ctx_t *ctx, ngx_rtmp_amf_elt_t *elts,
  ...
  for (n = 0; n < nelts; ++n) {
    ...
    if (type & NGX_RTMP_AMF_TYPELESS) {
      ...
      type8 = (uint8_t)type;
      if (ngx_rtmp_amf_put(ctx, &type8, 1) != NGX_OK) return NGX_ERROR;
    }

    switch (type) {
      case NGX_RTMP_AMF_NUMBER:
        if (ngx_rtmp_amf_put(ctx, ngx_rtmp_amf_reverse_copy(buf, data, 8), 8) !=
            NGX_OK) {
          return NGX_ERROR;
        }
        break;
      ...
      case NGX_RTMP_AMF_INT16:
        if (ngx_rtmp_amf_put(ctx, ngx_rtmp_amf_reverse_copy(buf, data, 2), 2) !=
            NGX_OK) {
          return NGX_ERROR;
        }
        break;
      ...
      case NGX_RTMP_AMF_INT32:
        if (ngx_rtmp_amf_put(ctx, ngx_rtmp_amf_reverse_copy(buf, data, 4), 4) !=
            NGX_OK) {
          return NGX_ERROR;
        }
        break;
