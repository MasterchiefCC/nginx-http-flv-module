...
static ngx_int_t ngx_rtmp_flv_init_index(ngx_rtmp_session_t *s,
                                         ngx_chain_t *in) {
  ...
  ngx_log_debug0(NGX_LOG_DEBUG_RTMP, s->connection->log, 0, "flv: init index");
  ...
  if (ngx_rtmp_receive_amf(s, in, in_elts,
    ...
    ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "flv: init index error");
    return NGX_OK;
  }
...
static ngx_int_t ngx_rtmp_flv_timestamp_to_offset(ngx_rtmp_session_t *s,
  ...
  for (index = 0; index < nelts - 1; ++index) {
    v = ngx_rtmp_flv_index_value(ngx_rtmp_flv_buffer + index * 9 + 1) * 1000;
...
static ngx_int_t ngx_rtmp_flv_send(ngx_rtmp_session_t *s, ngx_file_t *f,
  ...
  if (ctx->offset == -1) {
    ctx->offset = ngx_rtmp_flv_timestamp_to_offset(s, f, ctx->start_timestamp);
