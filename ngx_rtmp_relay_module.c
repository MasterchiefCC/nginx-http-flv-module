...
static ngx_int_t ngx_rtmp_relay_get_peer(ngx_peer_connection_t *pc,
                                         void *data) {
  return NGX_OK;
}
...
#undef NGX_RTMP_RELAY_STR_COPY
  ...
  if (addr->sockaddr->sa_family == AF_UNIX) {
    ...
    *ngx_cpymem(client, rctx->url.data, ngx_strlen(rctx->url.data)) = 0;
  ...
  addr_conf->default_server =
      ngx_pcalloc(pool, sizeof(ngx_rtmp_core_srv_conf_t));
...
static ngx_int_t ngx_rtmp_relay_create(
    ngx_rtmp_session_t *s, ngx_str_t *name, ngx_rtmp_relay_target_t *target,
...
ngx_int_t ngx_rtmp_relay_push(ngx_rtmp_session_t *s, ngx_str_t *name,
  ...
  return ngx_rtmp_relay_create(s, name, target, ngx_rtmp_relay_create_local_ctx,
                               ngx_rtmp_relay_create_remote_ctx);
}
...
static ngx_int_t ngx_rtmp_relay_play(ngx_rtmp_session_t *s,
                                     ngx_rtmp_play_t *v) {
  ngx_rtmp_relay_app_conf_t *racf;
  ngx_rtmp_relay_target_t *target, **t;
  ngx_str_t name;
...
static ngx_int_t ngx_rtmp_relay_send_connect(ngx_rtmp_session_t *s) {
  ...
  static ngx_rtmp_amf_elt_t out_cmd[] = {
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("serverName"), NULL, 0}, /* <-- fill */
...
static ngx_int_t ngx_rtmp_relay_on_result(ngx_rtmp_session_t *s,
  ...
  static ngx_rtmp_amf_elt_t in_inf[] = {
      ...
      {NGX_RTMP_AMF_STRING, ngx_string("description"), &v.desc, sizeof(v.desc)},
  };
...
static void ngx_rtmp_relay_close(ngx_rtmp_session_t *s) {
  ...
  ngx_rtmp_notify_ctx_t *nctx;
  ngx_uint_t hash, i;
  ngx_rtmp_session_t *ss, **temp, **cur;
  ngx_array_t *buffer;
  racf = ngx_rtmp_get_module_app_conf(s, ngx_rtmp_relay_module);

  nctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_notify_module);
  ctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_relay_module);
  if (ctx == NULL) {
    return;
  }
  ...
  if (nctx && nctx->reconnect_evt.timer_set) {
    ngx_del_timer(&nctx->reconnect_evt);
  }
  ...
  if (ctx->publish != ctx) {
    ...
    if (ctx->publish->play == NULL && ctx->publish->session->relay) {
      ngx_log_debug2(NGX_LOG_DEBUG_RTMP, ctx->publish->session->connection->log,
                     0, "relay: publish disconnect empty app='%V' name='%V'",
                     &ctx->app, &ctx->name);
  ...
  ///加加加
  buffer =
      ngx_array_create(s->connection->pool, 1, sizeof(ngx_rtmp_session_t *));
  for (cctx = &ctx->play; *cctx;) {
    ss = (*cctx)->session;
    (*cctx)->publish = ctx;
    temp = ngx_array_push(buffer);
    *temp = ss;
    ngx_rtmp_disconnect(ss);
    if (*cctx == NULL) break;
  ...
  for (i = 0; i < buffer->nelts; ++i) {
    cur = temp + i;
    if (ngx_rtmp_peer_connect_from_addrs(*cur) != NGX_OK) {
      ngx_rtmp_finalize_session(*cur);
    }
  }
...
#undef NGX_RTMP_RELAY_NUM_PAR
  ...
  if (is_static) {
    if (!is_pull) {
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "static push is not allowed");
      return NGX_CONF_ERROR;
    }
